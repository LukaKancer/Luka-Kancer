    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            if checkArray (Melee, death) then
                TriggerServerEvent('playerDied', killername .. " spojen " .. playerName)
            elseif checkArray (Bullet, death) then
                TriggerServerEvent('playerDied', killername .. " upucan " .. playerName)
            elseif checkArray (Knife, death) then
                TriggerServerEvent('playerDied', killername .. " izboden " .. playerName)
            elseif checkArray (Car, death) then
                TriggerServerEvent('playerDied', killername .. " upucan " .. playerName)
            elseif checkArray (Animal, death) then
                TriggerServerEvent('playerDied', playerName .. " ubijen od zivotinje")
            elseif checkArray (FallDamage, death) then
                TriggerServerEvent('playerDied', playerName .. " pao sa visine")
            elseif checkArray (Explosion, death) then
                TriggerServerEvent('playerDied', playerName .. " ubijen od eksplozije")
            elseif checkArray (Gas, death) then
                TriggerServerEvent('playerDied', playerName .. " umro od plina/benzina")
            elseif checkArray (Burn, death) then
                TriggerServerEvent('playerDied', playerName .. " spaljen do smrti")
            elseif checkArray (Drown, death) then
                TriggerServerEvent('playerDied', playerName .. " utopio se")
            else
                TriggerServerEvent('playerDied', playerName .. " je ubijen")
                --print(death)
            end
